...
#define BPB_BytsPerSec		11		/* Sector size [byte] (2) */ //我计算出来的为0x0200
#define BPB_SecPerClus		13		/* Cluster size [sector] (1) *///我计算出来的为0x01
#define BPB_RsvdSecCnt		14		/* Size of reserved area [sector] (2) *///我计算出来的为0x0021
#define BPB_NumFATs			  16		/* Number of FAT copies (1) *///我计算出来的为0x01
#define BPB_RootEntCnt		17		/* Number of root directory entries for FAT12/16 (2) *///我计算出来的为0x0000
#define BPB_TotSec16		  19		/* Volume size [sector] (2) *///我计算出来的为0x0000
#define BPB_Media			  21		/* Media descriptor (1) *///我计算出来的为0xf8
#define BPB_FATSz16			22		/* FAT size [sector] (2) *///我计算出来的为0x0000
#define BPB_SecPerTrk		24		/* Track size [sector] (2) *///我计算出来的为0x003f
#define BPB_NumHeads		26		/* Number of heads (2) *///我计算出来的为0x00ff
#define BPB_HiddSec			28		/* Number of special hidden sectors (4) *///我计算出来的为0x0000003f
#define BPB_TotSec32		32		/* Volume size [sector] (4) *///我计算出来的为0x0001ffc1
#define BS_DrvNum			  36		/* Physical drive number (2) *///我计算出来的为0x0004
...
#endif
	do {
		res = move_window(dp->fs, dp->sect);//注意就在下三句上c=0后，出现了res=FR_NO_FILE的情况，没有继续往下执行！！！added by sjw  ?????????????????????????
		if (res != FR_OK) break;
		dir = dp->dir;					/* Ptr to the directory entry of current index */
		c = dir[DIR_Name];
		if (c == 0) { res = FR_NO_FILE; break; }	/* Reached to end of table *///执行到这来，我看到c=0了，为什么？？？？？？？？？？？？？？？？？？？？？ 因为此时无这个文件！！！！！！！！！！！！
...
static
FRESULT create_name (//仿真确实在地址0x0800 641a的地方发现了文件srcfile.txt           added by sjw
	DIR* dp,			/* Pointer to the directory object */
...
#else
	if (*path == '/' || *path == '\\')		/* Strip heading separator if exist *///确实发现文件srcfile.txt   added by sjw
		path++;
			...
			if (res != FR_OK) {				/* Failed to find the object */
				if (res == FR_NO_FILE) {	/* Object is not found */
					if (_FS_RPATH && (ns & NS_DOT)) {	/* If dot entry is not exist, */
						...
						if (!(ns & NS_LAST)) res = FR_NO_PATH;	/* Adjust error code if not last segment *///直接跳出来了，此时还是  FR_NO_FILE??????????????????????????????
					}
				}
				break;
			}
...
/*
壹点灵异 
https://www.cnblogs.com/skullboyer/p/7807541.html-----------解释的函数get_ldnumber很好，值得借鉴！！
...
)
{
	...
	if (*path) {	/* If the pointer is not a null */  //判断 path 的内容是否为空字符 NULL
		//检测冒号 ‘：’，for循环的条件为 路径名中的字符 >= (_USE_LFN ? ' '： '!') 且 != ‘：’，只要_USE_LFN>0则第一个条件恒满足，找到冒号‘：’后则第二个条件失败，即跳出循环
		for (tt = *path; (UINT)*tt >= (_USE_LFN ? ' ' : '!') && *tt != ':'; tt++) ;	/* Find ':' in the path */
		if (*tt == ':') {	/* If a ':' is exist in the path name */ //再次确认当前路径字符为冒号‘：’
			tp = *path;//暂存*path
			i = *tp++ - '0'; //得到路径中的首字符那个数字字符对应的数字
			if (i < 10 && tp == tt) {	/* Is there a numeric drive id? */  //判断该数字在0~9之中，且路径中的第二个字符为冒号‘：’
				if (i < _VOLUMES) {	/* If a drive id is found, get the value and strip it *///判断该数字在定义的范围内
					vol = (int)i;//将该数字赋值给vol
					*path = ++tt;
				}
...
)
{
	...
	*rfs = 0;
	...
	fs = FatFs[vol];					/* Get pointer to the file system object *///AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA
	...
	*rfs = fs;							/* Return pointer to the file system object */
		...
		&& move_window(fs, bsect + 1) == FR_OK)
	{
		...
		if (LD_WORD(fs->win+BS_55AA) == 0xAA55	/* Load FSINFO data if available */
			&& LD_DWORD(fs->win+FSI_LeadSig) == 0x41615252 //AaRR        added by sjw
			&& LD_DWORD(fs->win+FSI_StrucSig) == 0x61417272)//aArr       added by sjw
		{
#if (_FS_NOFSINFO & 1) == 0
...
)
{
	...
	vol = get_ldnumber(&rp);//代码中第x行 vol = get_ldnumber(&rp) 将路径转换为 logical drive number
	if (vol < 0) return FR_INVALID_DRIVE;
	cfs = FatFs[vol];					/* Pointer to fs object *///BBBBBBBBBBBBBBBBBBBBBBBBB
	...
	FatFs[vol] = fs;					/* Register new fs object *///CCCCCCCCCCCCCCCCCCCCCCCCCCC |||||||||||||||||||||||||||||||||||||||||
...
//FindFirstVolume系列函数遍历驱动器，获取驱动器信息    added by sjw
//https://blog.csdn.net/diaoqu4574/article/details/102222676
...
//https://blog.csdn.net/PoJiaA123/article/details/53750109?spm=1001.2101.3001.6650.4&utm_medium=distribute.pc_relevant.none-task-blog-2%7Edefault%7ECTRLIST%7Edefault-4-53750109-blog-106243899.pc_relevant_multi_platform_whitelistv1&depth_1-utm_source=distribute.pc_relevant.none-task-blog-2%7Edefault%7ECTRLIST%7Edefault-4-53750109-blog-106243899.pc_relevant_multi_platform_whitelistv1
//FAT32 R0.12b f_mkfs应用实例 上面这个网址是针对R0.12b的



//https://www.cnblogs.com/amanlikethis/p/3793077.html#lab5
//FATFS API的函数功能简述

//FATFS 初学之 f_mkfs
//https://blog.csdn.net/weixin_30698527/article/details/97485325
//https://blog.csdn.net/weixin_30698527/article/details/97485325
FRESULT f_mkfs (
	const TCHAR *path,	/* Logical drive number 磁盘号*/
	BYTE sfd,			/* Partitioning rule 0:FDISK, 1:SFD */
	UINT au				/* Allocation unit [bytes]格式化的时候，要设置的簇的大小，以字节为单位 */
)
{
	...
	fs = FatFs[vol];//DDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDD
	...
	if (disk_ioctl(pdrv, GET_SECTOR_SIZE, &SS(fs)) != RES_OK || SS(fs) > _MAX_SS || SS(fs) < _MIN_SS)//获取块的大小，块是擦除的最小单位 added by sjw
		return FR_DISK_ERR;
#endif
	if (_MULTI_PARTITION && part) {
		...
		if (disk_ioctl(pdrv, GET_SECTOR_COUNT, &n_vol) != RES_OK || n_vol < 128)
			return FR_DISK_ERR;
		b_vol = (sfd) ? 0 : 63;		/* Volume start sector *///卷起始扇区 added by sjw
	...
	au /= SS(fs);		/* Number of sectors per cluster 一个簇占多少个扇区！added by sjw*/
	if (au == 0) au = 1;
	if (au > 128) au = 128;
	...
	n = get_fattime();						/* Use current time as VSN */  //我仿真得到n值为   0x44E2 0000
	if (fmt == FS_FAT32) {//仿真到此进行判断，我这里没有跳到FAT32里面，却跳到else部分了，？？？？？？？？？？？？？？？？？？？？？？？？
	...
	/* Initialize FAT area *///就是因为程序在4180行跳出了，确定没有对FAT area初始化，所以不正确！added by sjw
	wsect = b_fat;
...
#if _USE_ERASE	/* Erase data area if needed */
	{
		...
		disk_ioctl(pdrv, CTRL_ERASE_SECTOR, eb);//我有疑问的地方，就在此函数中第一次出现res=RES_PARERR;  的情况！！added by sjw????????????????????????????????????????????
	}
#endif

	/* Create FSINFO if needed */
	if (fmt == FS_FAT32) {
		ST_DWORD(tbl+FSI_LeadSig, 0x41615252);//AaRR
		ST_DWORD(tbl+FSI_StrucSig, 0x61417272);//aArr
