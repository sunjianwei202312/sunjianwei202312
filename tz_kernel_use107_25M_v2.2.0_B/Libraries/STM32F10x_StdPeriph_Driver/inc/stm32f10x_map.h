...
//#include "cortexm3_macro.h"  cancel by sjw 
#include  "core_cm3.h"
...
//typedef struct
//{
//  vu32 SR;//ADC状态寄存器  ------------STRT  JSTRT  JEOC	 EOC  AWD
//  vu32 CR1;//ADC控制寄存器1--AWDEN  AWDENJ * DUALMOD[3:0]  DISCNUM[2:0]	 DISCENJ  EISCEN	 JAUTO AWDSGL SCAN JEOCIE AWDIE EOCIE AWDCH[4:0]
//  vu32 CR2;//ADC控制寄存器2--TSVREFE SWSTART SWSTARTJ EXTTRIG EXTSEL[2:0]	* 
//  vu32 SMPR1;
//  vu32 SMPR2;
//  vu32 JOFR1;
//  vu32 JOFR2;																								  
...
/*typedef struct
{
  vu32 TIR;//发送邮箱标识符寄存器，其第0为由软件置1后，来请求发送邮箱x中的数据
  vu32 TDTR;//发送邮箱数据长度和时间戳寄存器，一个报文包含0-8个字节数据，这由其最低4位配置
  vu32 TDLR;//发送邮箱低字节数据寄存器 ，一个报文包含0-8个字节数据，且从字节0开始，一直到字节7结束！
...
typedef struct
{
  vu32 MCR;//can主控制寄存器-----------------------TTCM ABOM  AWUM  NART RFLM TXFP SLEEP INRQ
  vu32 MSR;//CAN主状态寄存器-----------------------RX SAMP RXM TXM - - -SLAKI WKUI ERRI SLAK INAK
  vu32 TSR;
  vu32 RF0R;
  vu32 RF1R;
  vu32 IER;
...
typedef struct
{
  vu32 CR; //时钟控制寄存器！ //HSI,HSE,CSS,PLL等的使能
  vu32 CFGR;//时钟配置寄存器！//PLL等的时钟源选择以及分频系数设定
  vu32 CIR;	//时钟中断寄存器！// 清除/使能时钟就绪中断

  vu32 APB2RSTR;//APB2外设复位寄存器！//APB2线上外设复位寄存器
  vu32 APB1RSTR;//APB1外设复位寄存器！//APB1线上外设复位寄存器

  vu32 AHBENR; //AHB外设时钟使能寄存器！//DMA，SDIO等时钟使能

  vu32 APB2ENR;	 //APB2外设时钟使能寄存器！//APB2线上外设时钟使能

  vu32 APB1ENR;	 //APB1外设时钟使能寄存器！//APB1线上外设时钟使能

  vu32 BDCR;		//备份域控制寄存器！ //备份域控制寄存器
