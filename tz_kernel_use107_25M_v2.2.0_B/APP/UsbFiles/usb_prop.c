...
void MASS_Init()
{
  ...
  _SetISTR(0);//清除挂起的中断
  wInterrupt_Mask = IMR_MSK;//设置中断屏蔽
  /* set interrupts mask */
...
void MASS_Reset()
{
  ...
  pInformation->Current_Feature = MASS_ConfigDescriptor[7];//当前特征！added by sjw

  SetBTABLE(BTABLE_ADDRESS);  //设置分组缓冲区描述表起始地址

  /* Initialize Endpoint 0 */
  SetEPType(ENDP0, EP_CONTROL);	  	  //设置为控制端点  //端点0为控制端点
  SetEPTxStatus(ENDP0, EP_TX_NAK);	  //设置发送端点状态//端点状态为发送无效，也就是主机IN令牌包来的时候，回送一个STALL??   is NAK
  SetEPRxAddr(ENDP0, ENDP0_RXADDR);	  //设置接收缓冲区地址//设置端点0的描述符表，包括接收缓冲区地址，最大允许接收的字节数、发送缓冲区地址三个量
  SetEPRxCount(ENDP0, Device_Property.MaxPacketSize);//接收缓冲区支持64个字节
  SetEPTxAddr(ENDP0, ENDP0_TXADDR);	  //设置发送缓冲区地址//发送缓冲区地址
  ...
  SetEPType(ENDP1, EP_BULK);	        //设置为批量端点  //端点1设为中断端点，在该。c文件中为EP_BULK
  SetEPTxAddr(ENDP1, ENDP1_TXADDR);	  //发送缓冲区地址  //设置发送缓冲区地址
