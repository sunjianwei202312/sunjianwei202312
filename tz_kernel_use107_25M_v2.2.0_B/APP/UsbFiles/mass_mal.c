...
u16 MAL_Write(u8 lun, u32 Memory_Offset, u32 *Writebuff, u16 Transfer_Length)
{
  ...
  switch (lun)
  {
    case 1://changed by sjw because only have a NANDFLASH!

      SD_WriteSingleBlock(Memory_Offset/512, (u8*)Writebuff);

      break;

    case 0://changed by sjw because only have a NANDFLASH!
...
u16 MAL_GetStatus (u8 lun)
{
 ...
 //   u32 Capacity;
  ...
  {
   //计算磁盘大小   ＝实际大小－备用块大小
      Mass_Block_Count[1] = FLASH_MAX_SECTOR_ADDR/FLASH_SECTOR_SIZE; //NAND_ZONE_SIZE * NAND_BLOCK_SIZE * xNAND_MAX_ZONE ;
