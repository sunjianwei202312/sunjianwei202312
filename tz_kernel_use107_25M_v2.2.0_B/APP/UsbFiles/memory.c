...
//USB协议中规定了一类大容量存储设备 mass storage device，
//包括u盘、移动硬盘等。大容量存储设备接口类代码（bInterfaceClass）为0x08，U盘接口子类代码（bInterfaceSubClass）使用0x06，

//ulk-only Mass Storage Reset请求是通知设备接下来的批量端点输出数据为命令快封装包CBW（Command Block Wrapper），
//在这个请求中，仅需要设置一下状态，说明接下来的数据是CBW，然后返回一个长度为0的状态数据包。
