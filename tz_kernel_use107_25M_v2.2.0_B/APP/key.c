...
*/
   ...
   write_hd7279(DECODE0+0,Subclock_State);   //某一面的状态
   write_hd7279(DECODE0+1,0x00);
   write_hd7279(DECODE0+2,0x0a); //
   write_hd7279(DECODE0+3,Subclock_Number);  //某一面的子钟
...
//SubclockChangeSta();	
            ...
            DelayMs(100);
				  ...
				  #elif   USE_MOTOR_THREE
					if(Subclock_Number==3 && Subclock3.warning==0){
						 switch (Subclock_State)
						 {
							 case 0 :
				...
				#else
				if(Subclock_Number==4 && Subclock4.warning==0){
						 switch (Subclock_State)
						 {
							 case 0 :
  ...
  if(ReadKey_Value==5){  功能确定   S6 	
	...
	*/
	 if(ReadKey_Value==7){  // 往右边移动，值以此为：0x3f  0xcf  0xf3   0xfc
				 ...
				 }else if(Blink_Number==0xfc){ //子钟 秒   ，秒为00或者30秒，不能++  added by sjw    
/*					 
							 Time_Modify[0]++; 
							 if(Time_Modify[0]>=10){	
								 Time_Modify[0]=0;         
