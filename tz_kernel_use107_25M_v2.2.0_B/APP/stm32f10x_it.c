...
uint8_t    ItWaitFlag,ItWaitFlag3;
...
void SysTick_Handler(void){ //1毫秒系统中断一次！参考  OS_TICKS_PER_SEC   定义
	OS_CPU_SR  cpu_sr;	   
	static INT16U  jsrun,jsrun2,jsrun3;
	...
	static	INT16U  once_js_rseg=0;
      ...
      ItWaitFlag=1;			
		...
		jsrun3++;
	  if(jsrun3>1999){//1000ms可以显示一次，否则芯片保护全部熄灭！！added  by sjw
			jsrun3=0;
      ItWaitFlag3=1;			
		}
		...
		if(!PowerOnFLag){//上电追时结束后才能开始走步进行判断！！added by sjw	
				...
				if((p_timemu->second==30 || p_timemu->second==53))
			  {
