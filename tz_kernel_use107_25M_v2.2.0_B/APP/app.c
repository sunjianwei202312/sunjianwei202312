...
//´ËÈÎÎñÓÃÀ´µ÷ÊÔÒ»Ð©×´Ì¬Î»£¬ÔÚÉÏÎ»»úÀïÖ±¹Û¼ì²â£¡£¡
...
{	
 ...
 static INT8U rxjs=0x11;	
	...
	ËùÒÔ£¬ÔÚ±àÐ´³ÌÐòÊ±£¬×î°²È«µÄ·½·¨£¬¾ÍÊÇ¾¡¿ÉÄÜ²»È¥Ê¹ÓÃ strcpy º¯Êý¡£¿ÉÒÔÔÚ³ÌÐòµÄ¿ªÍ·¼ÓÉÏ #define strcpy Unsafe_strcpy¡£ÕâÑù£¬
	¾Í»áÊ¹ strcpy º¯ÊýÔÚ±àÒëÊ±²úÉú´íÎó£¬´Ó¶øÊ¹ÎÒÃÇÔÚ±à³ÌÊ±¿ÉÒÔÍêÈ«ÞðÆústrcpy º¯Êý¡£ÔÚÍêÈ«¶ªÆú strcpy º¯ÊýµÄÍ¬Ê±£¬Ò²¾Í¶ªµôÁËÖÚ¶àÒÀ¸½ÓÚ strcpy º¯ÊýµÄ bug¡£
	*/
		...
		while(1){
			...
			//rxjs=SPI_I2S_ReceiveData(SPI1);
			     ...
			     if(ItWaitFlag3==1){
							//USART_SendData(USART2,Key_Subclock_State1); //·¢ËÍÒ»´Îµ±Ç°×´Ì¬!
						  printf("\r\n µç»ú×´Ì¬Îª£º%d ",Key_Subclock_State1);
						  printf("³õÊ¼ÉÏµçÎª£º%d ",PowerOnFLag);
						  printf("Õý×ª±êÖ¾£º%d ", MotorZZ_Flag);
						  printf("·´×ª±êÖ¾£º%d ", MotorFZ_Flag);
						  printf("\r\n GPS×´Ì¬£º%d ,%d ,%d ,%d", board.refeid[0],board.refeid[1],board.refeid[2],board.refeid[3]);
						  ItWaitFlag3=0;
						 // while((*r++ = *source++)!='\0');while((*des++=*source++));µÄ½âÊÍ£º ¸³Öµ±í´ïÊ½·µ»Ø×ó²Ù×÷Êý£¬ËùÒÔÔÚ¸³Öµ'\0'ºó£¬Ñ­»·Í£Ö¹¡£
					 }
		 #endif
...
//´ËÈÎÎñ×¨ÃÅ´¦Àí¼üÅÌ¿ì×·£¨Õý×ª»òÕß·´×ª¹¦ÄÜ£¡£¡£©added by sjw
...
//´ËÈÎÎñÔÚÉè¶¨µÄÊ±¼ä¶ÁÈ¡Ò»´ÎDS3231ÀïÃæµÄÊ±¼ä£¡
static  void AppTask8(void* p_arg)
{
	INT8U  err;
	static INT8U first_flag;
